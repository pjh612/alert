...
public class AlertKafkaConfig {
    ...
    public AlertKafkaConfig(AlertProperties alertProperties, AlertKafkaProperties alertKafkaProperties) {
    ...
    public MessageConsumerRegistrar messageConsumerRegistrar(ConcurrentKafkaListenerContainerFactory<String, AlertMessage> alertKafkaListenerContainerFactory, ApplicationContext applicationContext,
        ...
        KafkaMessageConsumerRegistrar kafkaMessageConsumerRegistrar = new KafkaMessageConsumerRegistrar(alertKafkaListenerContainerFactory, applicationContext, errorHandler);
        List<String> topics = alertProperties.topics();
        for (String topic : topics) {
            kafkaMessageConsumerRegistrar.register(topic, new AlertMessageBroadcastHandler(messageBroadcaster, objectMapper, basicSender));
        }
